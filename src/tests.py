...
def assert_almost_equal_relative(tolerance=0.01, rows=None):
    if len(rows) != 2:
        raise Exception("Almost Equal Relative Assertion works only on queries that return 2 exact rows and 1 column")
    val0 = rows[0][0]
    val1 = rows[1][0]
    reldiff = (val0 - val1) / max([val0, val1])
    assert -tolerance <= reldiff <= tolerance, \
        f"Values are too far apart. Tolerance {tolerance}, ({val0}-{val1})/{max([val0, val1])}={reldiff}"
